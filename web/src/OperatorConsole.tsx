...
function JobsPage({ initialJobs, expert }: { initialJobs: Job[]; expert: boolean }) {
  ...
  return <>
    ...
    {selected && <Section title={`Job ${selected.id}`} eyebrow="Inspection">
      ...
      {selected.state === "awaiting_golden_approval" && <div className="detail-panel"><h3>Golden update approval</h3><p>Changed or missing approved golden artifacts require explicit reviewer approval before documentation and QC continue.</p><label>Golden decision rationale<textarea required rows={2} maxLength={1000} value={rationale} onChange={(event) => setRationale(event.target.value)} /></label><label>Current reviewer password<input type="password" autoComplete="current-password" minLength={14} value={password} onChange={(event) => setPassword(event.target.value)} /></label>{(detail?.golden_rehearsal_reports ?? []).flatMap((report: any) => report.comparisons.filter((comparison: any) => comparison.approval_required).map((comparison: any) => <article className="finding-card" key={`${report.id}/${comparison.id}`}><header><Badge value={comparison.status} /><span>{comparison.id}</span></header><p>{comparison.diff_summary}</p><details><summary>Tolerance, masks, and artifact identities</summary><pre>{JSON.stringify({ tolerance_policy: comparison.tolerance_policy, mask_policy: comparison.mask_policy, approved_artifact_sha256: comparison.approved_artifact_sha256, candidate_artifact_sha256: comparison.candidate_artifact_sha256 }, null, 2)}</pre></details><div className="card-actions"><button type="button" onClick={() => void approveGoldenComparison(report.id, comparison.id, true)}>Approve update</button><button className="secondary-button" type="button" onClick={() => void approveGoldenComparison(report.id, comparison.id, false)}>Reject update</button></div></article>))}</div>}
