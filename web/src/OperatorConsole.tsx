...
function statusTone(value: string): "good" | "warn" | "bad" | "neutral" {
  ...
  if (["awaiting_operator", "awaiting_task_approval", "awaiting_test_design_disposition", "awaiting_golden_approval", "awaiting_remote_egress_approval", "awaiting_repair", "quarantine", "disputed", "delivered"].includes(value)) return "warn";
  return "neutral";
}
...
function JobsPage({ initialJobs, expert }: { initialJobs: Job[]; expert: boolean }) {
  ...
  const pendingRemoteEgress = selected?.state === "awaiting_remote_egress_approval" ? [...(detail?.transitions ?? [])].reverse().find((transition: any) => transition.to === "awaiting_remote_egress_approval")?.details : null;
  const approveRemoteEgress = async () => {
    if (!selected || !pendingRemoteEgress?.provider_egress_manifest_sha256 || !pendingRemoteEgress?.resume_state || !rationale.trim() || !password) { setMessage("Remote egress approval requires retained manifest evidence, a rationale, and current reviewer password."); return; }
    const auth = await api.POST("/auth/reauthenticate", { params: { header: { "X-CSRF-Token": getCSRFToken() } }, body: { password } });
    setPassword("");
    if (!auth.data) { setMessage("Reauthentication failed; remote egress was not approved."); return; }
    const response = await api.POST("/jobs/{jobID}/actions/approve-remote-egress", { params: { path: { jobID: selected.id }, header: { "X-CSRF-Token": getCSRFToken() } }, body: { rationale, manifest_sha256: pendingRemoteEgress.provider_egress_manifest_sha256, resume_state: pendingRemoteEgress.resume_state } });
    setMessage(response.data ? `Remote egress approved for manifest ${shortSHA(response.data.approval.subject_sha)}.` : "Remote egress approval failed; refresh and inspect the retained manifest.");
    if (response.data) { setRationale(""); await load(); await inspect(response.data.job); }
  };
  ...
  return <>
    ...
    {selected && <Section title={`Job ${selected.id}`} eyebrow="Inspection">
      ...
      {selected.state === "awaiting_remote_egress_approval" && <div className="detail-panel"><h3>Remote egress approval</h3><p>Network contact is withheld until a reviewer approves this exact per-job egress manifest.</p><dl><div><dt>Manifest</dt><dd><code>{shortSHA(pendingRemoteEgress?.provider_egress_manifest_sha256)}</code></dd></div><div><dt>Provider</dt><dd>{pendingRemoteEgress?.provider_id ?? "—"}</dd></div><div><dt>Model</dt><dd>{pendingRemoteEgress?.provider_model_profile_id ?? "—"}</dd></div><div><dt>Purpose</dt><dd>{pendingRemoteEgress?.provider_purpose ?? "—"}</dd></div><div><dt>Resume state</dt><dd><Badge value={pendingRemoteEgress?.resume_state ?? "unknown"} /></dd></div></dl><details open><summary>Data classes, redactions, retention, estimates, and policy reason</summary><pre>{JSON.stringify(pendingRemoteEgress, null, 2)}</pre></details><label>Reviewer rationale<textarea required rows={2} maxLength={4000} value={rationale} onChange={(event) => setRationale(event.target.value)} /></label><label>Current reviewer password<input type="password" autoComplete="current-password" minLength={14} value={password} onChange={(event) => setPassword(event.target.value)} /></label><button type="button" onClick={() => void approveRemoteEgress()}>Approve remote egress manifest {shortSHA(pendingRemoteEgress?.provider_egress_manifest_sha256)}</button></div>}
