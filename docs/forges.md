...
GitLab uses a file-backed token in `git-bridge`; it is sent only through request headers or temporary askpass transport. Diagnostics require Reporter-level read access and Developer-level draft-publication access. Discussion expansion is reported `operator_only` because it requires bounded per-issue and per-merge-request traversal. Webhook status remains polling/operator configuration until GitLab secret-token webhook normalization is enabled and validated.
...
```text
maintainctl forge save PROJECT_ID --file forge-profile.json
maintainctl forge show PROJECT_ID
...
The deterministic test path uses loopback fake GitHub/GitLab APIs and local bare Git. Real hosted credentials, provider editions, webhooks, and rate limits are operator-only validation and are not claimed by the local suite.
