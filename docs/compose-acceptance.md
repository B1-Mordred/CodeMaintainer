...
The browser gate exercises the Models and agents provider workbench, probes the fake remote model, enables the fake remote provider, approves a remote documentation route after retained egress preview review, registers the local fixture repository through the authenticated UI, queues and advances a provider-backed job, and verifies the completed job retained remote documentation provider execution evidence without contacting the network.
