...
- [ ] (2026-07-23 22:25Z) Increment 2 Milestone 5 agent-contract checkpoint: migration 26 adds append-only schema-validation evidence; implementation and QC model calls request strict JSON Schema response formats; workflow phases record schema/payload hashes; and REST/OpenAPI/generated-client/CLI/browser surfaces expose schema versions, roles, modes, hashes, retry posture, and validation history. Milestone 5 remains open for independent Test Designer routing, retry orchestration, golden/rehearsal gates, Documentation Agent/QC, and OPA-backed policy activation.
- [ ] (2026-07-23 23:10Z) Increment 2 Milestone 5 independent Test Designer checkpoint, paused and synchronized at operator request: migration 27 adds append-only Test Designer reports; medium/high-risk jobs route from full verification through `loading_test_designer_model` and `test_design_review` before QC; low-risk seeded phases store explicit skipped evidence. The Test Designer packet/output is exact contract/risk/result bound and visible through REST/OpenAPI/generated-client, `maintainctl test-designer`, and the Jobs page. Full Go tests, full race tests, Go vet, frontend API drift/tests/typecheck/build, Redocly lint, and Compose validation pass; disposition enforcement, golden/rehearsal, documentation QC, and OPA remain open.
- [ ] (2026-07-23 23:35Z) Increment 2 Milestone 5 golden/rehearsal foundation checkpoint: migration 28 adds append-only golden rehearsal reports and update approval records; the workflow inserts `golden_rehearsal_review` before QC after Test Designer or whenever low-risk work has registered pack rehearsals; reports select only trusted capability rehearsal definitions and matching Test Designer suggestions; changed goldens fail closed with approval-required evidence instead of updating themselves. REST/OpenAPI/generated-client, `maintainctl golden`, and the Jobs page expose comparisons, tolerance/mask metadata, provenance, and approvals. Focused Go tests, full Go tests, frontend API drift/tests/typecheck, and Redocly lint pass; rendered visual diffs, editable tolerance/mask management, documentation QC, and OPA remain open.
- [ ] (2026-07-23 23:39Z) Increment 2 Milestone 5 Documentation Agent foundation checkpoint: migration 29 adds append-only documentation manifests; `loading_documentation_model` and `documentation_review` now run after full/golden evidence and before QC; documentation manifests are exact job/project/contract/risk/result bound; documentation commits update `result_sha`, record fresh test-impact evidence, and run fresh full verification before QC. The `documentation` worker kind is digest-pinned and inference-only, mock profiles include deterministic source-controlled docs, and REST/OpenAPI/generated-client, `maintainctl documentation`, and the Jobs page expose requirements, changes, checks, edits, and unsupported claims. Focused Go and frontend/OpenAPI checks pass; full documentation policy/toolchain/QC, OPA, and later milestones remain open.
- [ ] (2026-07-24 00:18Z) Increment 2 Milestone 5 OPA policy lifecycle foundation checkpoint: migration 30 adds append-only policy bundles, activations, simulations, and decisions. The workflow inserts `policy_review` after Documentation Agent evidence and before QC model loading; the active structured policy bundle records redacted inputs, required stages, bundle version, allow/deny explanation, and fail-closed state. REST/OpenAPI/generated-client, `maintainctl policy`, and the Jobs page expose bundle, activation, simulation, and job decision evidence. Focused Go tests, full Go tests, Go vet, frontend API drift/tests/build, OpenAPI lint, and Compose validation pass; full embedded OPA/Rego authoring, policy tests/coverage UI, rollback workbench, and broader protected-action enforcement remain open.
- [ ] (2026-07-24 00:48Z) Increment 2 paused at an operator-requested sync point after the embedded OPA/Rego policy-test checkpoint: migration 31 adds append-only retained test runs; advanced bundles are formatted, source-hashed, evaluated through the embedded OPA library, and activation-gated by the latest retained passing test run for the exact formatted source. REST/OpenAPI/generated-client, `maintainctl policy create-advanced|test|test-runs`, and the Policy page expose source, structured interpretation, simulations, tests/coverage, activations, and expert Rego authoring. Focused Go, full Go, full race, Go vet, frontend API drift/tests/build, OpenAPI lint, and Compose validation pass; structured policy editing, richer staged rollout/rollback, Test Designer disposition enforcement, golden visual diffs/tolerance management, full documentation QC, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 01:09Z) Increment 2 Milestone 5 Test Designer disposition enforcement checkpoint: migration 32 adds append-only proposal dispositions; required pending proposals now stop in `awaiting_test_design_disposition` before golden, documentation, policy, or QC stages. Effective report views overlay the latest reviewer/admin decision while preserving the original report and every disposition event. REST/OpenAPI/generated-client, `maintainctl test-designer dispose`, and the Jobs page expose the action. Focused Go and frontend checks pass; golden visual diffs/tolerance management, full documentation QC, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 01:28Z) Increment 2 Milestone 5 agent-contract retry checkpoint: the model-backed implementation, QC, Test Designer, and Documentation Agent workers now enforce the registered structured-output retry policy. Invalid completions receive bounded controller validation feedback and retry up to three attempts; exhaustion fails closed before edits are applied or malformed evidence is accepted. Focused `internal/agents`, `internal/workflow`, and `cmd/maintainer-worker` tests pass; golden visual diffs/tolerance management, full documentation QC, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 01:37Z) Increment 2 Milestone 5 golden approval wait checkpoint: approval-required golden/rehearsal reports now route to non-resumable `awaiting_golden_approval` instead of failing the job. Latest append-only approval decisions resolve required comparisons, and the API advances a waiting job to documentation loading after every required comparison is approved. REST/OpenAPI/generated-client and the Jobs page expose approval/rejection controls. Focused Go and frontend checks pass; rendered visual diffs, editable tolerance/mask management, full documentation QC, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 01:53Z) Increment 2 Milestone 5 typed golden tolerance/mask profile checkpoint: migration 33 adds append-only golden comparison profiles; reviewer/admin operators can record typed absolute/relative tolerance, dynamic-region masks, and explicit mask selectors without mutating immutable reports or approvals. REST/OpenAPI/generated-client, `maintainctl golden configure-profile`, job-detail evidence, and the Jobs page expose profile history and a structured diff preview with summary/provenance/hash/policy evidence. Focused Go packages, frontend API drift/tests/build, and generated-client drift pass; richer rendered visual diff artifacts, full documentation QC, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 02:09Z) Increment 2 Milestone 5 documentation-policy finding lifecycle checkpoint: blocked Documentation Agent manifests now convert pending/blocked requirements, failed checks, unsupported claims, and generic blocked policy evidence into stable retained findings. The existing finding state machine handles dispute/accept/waive/escalate, and repair-cycle review permits only documentation policy categories rather than arbitrary new blockers. The Jobs page shows documentation finding counts and expert evidence alongside manifests. Focused workflow/storage tests and frontend API drift/tests/build pass; the declarative documentation policy builder, renderer/tool profiles, screenshot/golden management, rich render previews, independent documentation QC actions, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 02:30Z) Increment 2 Milestone 5 documentation policy simulation checkpoint: `internal/docagent` now exposes a controller-owned versioned documentation policy profile with typed rules, match criteria, tool profile metadata, source-of-truth mappings, reviewer roles, render targets, checks, and publication gates. REST/OpenAPI/generated-client, `maintainctl documentation policy|simulate`, and the Policy page expose bounded change-impact simulation from paths/classes/risk/languages/packs/labels without executing repository tools. Focused Go and frontend API drift/tests/build pass; persistent editable documentation policy/tool profiles, render previews, independent documentation QC actions, structured policy editing, richer rollout/rollback, and cross-domain protected-action enforcement remain open.
- [ ] (2026-07-24 03:25Z) Increment 2 paused at an operator-requested sync point after starting Milestone 6 with the advisory resource scheduler foundation. Migration 34 adds append-only retained scheduler decisions; the pure scheduler domain simulates bounded topology/profile, maintenance-window, fairness, memory, CPU, and model-batch choices; REST/OpenAPI/generated-client, `maintainctl scheduler`, and the Automation page expose status, simulation, and history. Focused Go and frontend checks pass; live queue-lease enforcement, provider-neutral inference, runtime lab, evidence graph, evaluation isolation, OpenTelemetry, and support bundles remain open.
- [ ] (2026-07-24 03:55Z) Increment 2 Milestone 6 live scheduler lease checkpoint: durable job lease acquisition now builds a scheduler request from active unexpired leases and resumable unlocked candidates in the same SQLite transaction, maps workflow states to bounded controller/dependency/verification/model profiles, records selected/deferred decisions append-only, defers same-project concurrent candidates for fairness, and refuses candidates whose declared profile would exceed CPU or memory capacity after active leases and controller reservation. Focused scheduler/storage/queue tests, full Go tests, and Go vet pass; richer resource/provider inputs and the rest of Milestone 6 remain open.
- [ ] (2026-07-24 04:20Z) Increment 2 Milestone 6 provider-gateway foundation checkpoint: migration 35 adds durable provider, endpoint, model, route, and append-only egress-manifest records; default profiles keep remote inference disabled while preserving local llama.cpp routing and fake remote family templates; the provider service validates endpoint network/TLS/SSRF policy, data classes, structured-output capability, token/cost ceilings, and enabled route/model eligibility before retaining an allow/deny egress manifest. REST/OpenAPI/generated-client, `maintainctl model-provider`, the Models page, provider docs, and coverage evidence are current. Focused provider/storage/API/CLI tests and targeted frontend API/test/build gates pass; real provider adapters, credential rotation, drift handling, worker integration, runtime lab, evidence graph, evaluation isolation, OpenTelemetry, and support bundles remain open.
- [ ] (2026-07-24 04:25Z) Increment 2 paused at the operator-requested sync checkpoint after adding provider capability probes: migration 36, deterministic fake adapters for all required provider families, retained native API-shape and schema-hash observations, API/OpenAPI/generated-client/CLI probe operations, Models-page probe controls/history, updated provider docs, and coverage evidence. Focused provider/storage/API/CLI and frontend API/App tests pass; broad validation and git synchronization are the checkpoint gate.
- [ ] (2026-07-24 04:42Z) Increment 2 Milestone 6 runtime benchmark lab checkpoint: migration 37 stores append-only local runtime benchmark runs; the allow-listed model benchmark action now retains exact runtime identity, smoke quality/determinism gates, speed, memory, cache eligibility, experimental feature gates, and candidate/rejected recommendation evidence without activating profiles or optimizations. REST/OpenAPI/generated-client, `maintainctl model benchmarks`, Models-page runtime lab, docs, and coverage evidence are current. Focused runtime/storage/API/CLI and frontend API/App tests pass; activation/rollback and richer fixture coverage remain open.
...
- [ ] (2026-07-24 09:05Z) Increment 2 Milestone 7 DOD-13 verification-safety checkpoint: `config/verification-safety-coverage.json` now maps non-waivable new failures, mandatory final full-suite evidence, explicit golden-update approval, and operator parity to implementation, tests, browser/CLI/API coverage, docs, and proof snippets.
- [ ] (2026-07-24 09:25Z) Increment 2 Milestone 7 DOD-14 documentation-policy checkpoint: `config/documentation-policy-coverage.json` now maps policy impact selection, source-controlled Documentation Agent generation, render/check validation blockers, fresh verification/traceability, and operator parity to implementation, tests, browser/CLI/API coverage, docs, and proof snippets, with remaining documentation QC/tool-profile work explicit.
- [ ] (2026-07-24 09:45Z) Increment 2 Milestone 7 security/adversarial checkpoint: `config/security-adversarial-coverage.json` now maps authorization/reauthentication/stale-edit gates, untrusted-input injection rejection, worker isolation, provider SSRF/egress/fallback/cost controls, forge webhook authentication, cross-project isolation, backup restore path safety, no-write previews, and redaction to implementation, tests, browser/CLI/API coverage, docs, and proof snippets. Backup restore also now has an explicit unsafe pending-path regression.
- [ ] (2026-07-24 10:05Z) Increment 2 Milestone 7 unit/property/fuzz checkpoint: `config/unit-property-fuzz-coverage.json` maps 31.2 categories to existing domain tests and new fuzz entrypoints for declarative imports, model manifests, webhooks, backup restore paths, untrusted Markdown task packets, and fake-model packet extraction. Streamed-provider-event fuzzing remains explicit unfinished provider-gateway work.
- [ ] (2026-07-24 10:40Z) Increment 2 Milestone 7 fixture/integration failure checkpoint: `config/integration-failure-coverage.json` maps 31.3 fixture repositories and failure modes to checked proof snippets. The PHP fixture now includes npm/Vite/Tailwind files, and provider routing has direct denied-manifest regression coverage for cost-circuit and capability-drift cases.
- [ ] (2026-07-24 11:15Z) Increment 2 Milestone 7 local-fake E2E checkpoint: `config/local-fake-e2e-coverage.json` maps all 14 required local-fake workflow steps to checked implementation, browser, API/CLI, docs, and proof snippets. The deterministic coordinator test now asserts retained evidence-graph nodes and edges for the local publish workflow. `I2-31.4` remains in progress because several browser-originated/provider/restore steps are proven by focused tests rather than one continuous browser-to-controller run.
- [ ] (2026-07-24 11:45Z) Increment 2 Milestone 7 performance/soak checkpoint: `config/performance-soak-coverage.json` maps scheduler resource safety, runtime benchmark gates, historical evaluation metrics, observability timing/resource records, deterministic acceptance smoke evidence, and remaining bounded-soak report work to checked proof snippets. `I2-31.5` remains in progress because true long-duration soak thresholds and retained pass/fail reports are still not implemented.
- [ ] (2026-07-24 12:05Z) Increment 2 Milestone 7 runnable Compose/acceptance checkpoint: `config/compose-acceptance-coverage.json` maps CI, local acceptance, documented Compose views, mock runtime stack startup/health, and maintenance scripts to checked proof snippets. `I2-DOD-26` remains in progress because the full live acceptance script still needs a retained final run after the remaining local-fake E2E, soak, and final-acceptance rows close.

## Surprises & Discoveries

- Observation: golden tolerance and mask changes were only static JSON embedded in immutable report comparisons.
  Evidence: migration 33 stores reviewer/admin `golden_comparison_profiles` separately from `golden_reports` and `golden_approvals`; the API and CLI tests record a typed profile without rewriting the original report.
- Observation: model-backed agents still use the internal HTTP v1 model endpoint directly, so the provider gateway is not yet the only runtime inference path.
  Evidence: `internal/agents/runner.go` contains `NewModelClient`; migration 35 and `internal/providers` add gateway profiles and egress decisions as the foundation for replacing those call paths in a later provider slice.
...
- Observation: the existing queue/schedule path retained dispatch history but not resource-decision reasoning.
  Evidence: migration 34 adds separate append-only `scheduler_decisions` rows for selected/deferred/rejected job IDs, co-residence safety, fairness, model batch group, and resource summary; the later lease checkpoint binds live acquisition to those decisions.
...
- Observation: embedded OPA was feasible without adding a policy sidecar.
  Evidence: the controller uses the pinned `github.com/open-policy-agent/opa/v1/format` and `github.com/open-policy-agent/opa/v1/rego` packages, retains the dependency graph in `go.mod`/`go.sum`, and full Go, race, vet, frontend, OpenAPI, and Compose checks passed after the integration.
- Observation: Test Designer reports needed separate disposition evidence rather than report mutation.
  Evidence: migration 27 retained proposals inside immutable report JSON; migration 32 adds a separate append-only disposition table and the service overlays the latest reviewer decision when reports are listed.
- Observation: migration and restore evidence was present but too indirect for final Increment 2 closure.
  Evidence: migration files existed through v40 and backup tests covered dry-run/apply, but the coverage map still marked the durable data model and backup/recovery DOD rows missing. The new migration coverage inventory and SQLite test require every embedded migration/file pair to name durable records, restart/resume evidence, backup/restore evidence, docs, and rollback guidance.
- Observation: final acceptance needed a checked distinction between fake CI proof and real operator-only infrastructure proof.
  Evidence: `docs/acceptance.md` listed the eight production checks, but no machine-readable artifact tied each item to safe fake evidence and a non-claim boundary. `config/external-validation-coverage.json` now records that mapping.
- Observation: setting-level configuration coverage did not prove every routine configuration operation remained reachable from REST, CLI, and the browser.
  Evidence: the descriptor inventory compared settings to evidence, but no checked artifact enumerated operation parity. `config/configuration-parity.json` now maps every typed registry and legacy compatibility operation to OpenAPI, `maintainctl`, UI, docs, and tests.
- Observation: the safe pause point was the green configuration-parity commit, before redaction coverage work started.
...
- Observation: the DOD-13 safety boundary crossed multiple subsystems without a single checked inventory.
  Evidence: intelligence tests rejected hiding new findings, workflow code required full-suite final impact evidence, golden tests required approval status, and operator surfaces existed; the Increment 2 coverage file still marked the row missing.
- Observation: DOD-14 had partial implementation evidence and partial remaining work but no checked acceptance boundary.
  Evidence: policy simulation, documentation manifests, blocked findings, source-controlled documentation commits, fresh verification, and UI/API/CLI surfaces exist; persistent editable documentation policy profiles, renderer/tool profiles, render previews, and independent documentation QC actions remain open.
...
- Observation: streamed-provider-event fuzzing cannot honestly be claimed yet.
  Evidence: the current model worker sends `"stream": false` and the fake model server rejects streaming requests; provider-family fake adapters retain capability probes but no streamed chunk parser exists to fuzz.
...
- Observation: provider-family probes are broader than provider execution failure fakes.
  Evidence: all required provider families have default fake profiles and retained probes, but the gateway still has no outbound execution seam where timeout, rate-limit retry, streamed interruption, or batch partial-failure cases can run.
- Observation: the deterministic coordinator E2E covers the repair/publication core but not every browser-originated local-fake contract step as one flow.
  Evidence: `TestCoordinatorCompletesImplementRejectRepairApproveAndLocalPublish` drives task approval, risk/context/test impact, deterministic implementation, Test Designer, verification, documentation, QC block/repair, local bare-Git publication, artifacts, memory, and evidence graph checks. Browser onboarding, pack acceptance/effective-config inspection, UI configuration rollback, fake remote route approval, lower-trust fallback, and backup/migrate/restore/resume are currently proven by focused UI/API/CLI/storage tests and are tracked as remaining work in `config/local-fake-e2e-coverage.json`.
- Observation: performance and soak evidence exists in scheduler, runtime benchmark, evaluation, and observability domains, but no final soak runner or retained threshold report exists yet.
  Evidence: scheduler tests prove bounded resource/fairness decisions, runtime benchmarks retain speed/memory/quality identity, evaluation records wall/CPU/memory/cache metrics, and observability retains duration/queue/resource facts. `config/performance-soak-coverage.json` now records those proofs and preserves the missing deterministic soak runner as explicit remaining work.
...
- Decision: keep external systems behind domain interfaces and ship deterministic fakes selected by the development configuration.
  ...
  Date/Author: 2026-07-20 / Codex
- Decision: use the embedded OPA Go library for advanced policy bundles and require retained tests for the exact formatted source before activation.
  Rationale: in-process policy evaluation keeps decisions transactional and auditable without introducing another privileged service boundary. Formatting and source hashes make activation evidence reproducible, and any OPA failure remains a fail-closed controller decision.
  Date/Author: 2026-07-24 / Codex
- Decision: add a dedicated Test Designer disposition wait state and an append-only disposition ledger.
  Rationale: required independent recommendations must be resolved before later quality gates, but the original report must remain immutable. A non-resumable wait state makes operator attention explicit and prevents silent progression.
  Date/Author: 2026-07-24 / Codex
- Decision: run schema-constrained output retries inside the trusted model worker before applying edits or publishing trusted agent artifacts.
  Rationale: response-format support is only a model/runtime constraint. The controller's exact domain validation catches task-binding, safety, and evidence invariants; invalid output can receive bounded feedback, but only a validated final JSON document may reach workflow storage.
  Date/Author: 2026-07-24 / Codex
- Decision: make changed-golden approval a non-terminal operator wait state resolved by latest append-only approval decisions.
  Rationale: golden changes are review decisions rather than worker failures. Waiting keeps the job auditable and resumable while still blocking later quality gates until every required comparison has an approved decision.
  Date/Author: 2026-07-24 / Codex
...
- Decision: reuse the existing QC finding lifecycle for documentation-policy blockers instead of adding a parallel documentation-only resolution table.
  ...
  Date/Author: 2026-07-24 / Codex
- Decision: treat migration evidence as a checked release artifact.
  Rationale: forward-only migrations, backup/recovery, and rollback instructions must advance together. `config/migration-coverage.json` gives CI a concrete object to compare against embedded migrations and files instead of relying on plan prose.
  Date/Author: 2026-07-24 / Codex
- Decision: keep external-only validation coverage explicit and testable.
  Rationale: CI must not imply that unavailable credentials, licensed systems, production identity, real hardware, or external scanners were exercised. A checked coverage artifact preserves the fake evidence and finite operator procedures for each boundary.
  Date/Author: 2026-07-24 / Codex
- Decision: treat configuration parity as operation coverage, not only descriptor coverage.
  Rationale: descriptors prove settings exist; the product DOD also requires operator workflows to be reachable through the web UI, API, and CLI. A checked parity matrix prevents a route or CLI command from drifting away from the browser workbench without evidence.
  Date/Author: 2026-07-24 / Codex
- Decision: pause before the cross-surface secret-redaction matrix.
  Rationale: redaction is a broad safety invariant and should land as a complete tested slice rather than partial uncommitted work after a green checkpoint.
  Date/Author: 2026-07-24 / Codex
...
- Decision: make DOD-13 a checked safety matrix across differential, final verification, and golden approval gates.
  Rationale: the requirement combines non-waiver guarantees from separate subsystems. A single proof inventory keeps future changes from weakening one gate while relying on unrelated green tests.
  Date/Author: 2026-07-24 / Codex
- Decision: make DOD-14 an in-progress checked matrix with explicit known remaining work.
  Rationale: the current implementation has enough durable behavior to verify but not enough to claim the full documentation-policy DOD. A matrix keeps current proof and missing renderer/QC/tool-profile work visible.
  Date/Author: 2026-07-24 / Codex
...
- Decision: if the Documentation Agent changes source-controlled documentation, commit it as ordinary repository content and rerun exact-commit verification before QC.
  ...
  Date/Author: 2026-07-23 / Codex
- Decision: start resource scheduling as an advisory append-only decision layer before replacing live queue leases.
  Rationale: this creates auditable topology/profile/fairness/co-residence evidence and operator UI parity at a safe pause boundary, while preserving the proven serial dispatcher until the next slice can bind leases transactionally to scheduler decisions.
  Date/Author: 2026-07-24 / Codex
- Decision: enforce scheduler decisions at job lease acquisition while continuing to use the existing workflow state machine.
  ...
  Date/Author: 2026-07-24 / Codex
- Decision: add provider profiles and egress-manifest routing before replacing all model worker calls.
  Rationale: route eligibility, data classification, endpoint admission, token/cost limits, and fail-closed evidence can be validated with fakes first. Moving worker traffic through the gateway should happen after provider-family conformance adapters and exact route snapshot storage exist, so the existing local mock workflow stays runnable.
  Date/Author: 2026-07-24 / Codex
...
Increment 2 Milestone 6 has started but remains open. The scheduler now has safe defaults, deterministic simulation, retained decision evidence, API/CLI/browser parity, and transactional live lease enforcement for fairness and declared CPU/RAM co-residence. The provider gateway now has versioned durable profile records, default-off remote templates for every required provider family, safe local routing, endpoint/data/capability/budget checks, retained hashed egress manifests, append-only deterministic fake capability probes, and API/CLI/browser parity. The runtime benchmark lab retains exact local profile identity, quality/determinism gates, speed, memory, cache eligibility, and non-activating recommendations. The evidence graph, historical evaluation lab, and first redacted local observability/support-bundle foundation now have durable API/CLI/browser/docs coverage. Real provider adapters, worker route snapshots, runtime activation/rollback, full request-through-publication/SBOM graph coverage, real historical repair execution, and full cross-component OpenTelemetry instrumentation remain before the milestone can close.
...
Increment 2 Milestone 5 is partially implemented and paused at an auditable sync point. Task contracts, deterministic risk routing, schema-constrained agent contracts with bounded validation-feedback retries, validation evidence, independent Test Designer proposal capture and disposition enforcement, golden/rehearsal report plus non-terminal approval-wait and typed tolerance/mask profile evidence, Documentation Agent manifest, documentation-policy finding, and documentation impact-simulation evidence, deterministic policy-review decisions, and embedded OPA/Rego test-gated policy bundles are durable and visible across API/CLI/browser surfaces; remaining quality policy work is deliberately unclaimed.

Increment 2 Milestone 7 has started but remains open. The operator console now uses the required 17 named areas, keeps setup/health/administration operations available, adds dedicated Documentation and Security/SBOM pages backed by controller-retained policy, manifest, capability-pack, finding, rehearsal, and release-evidence data, and links every non-configuration area to a filtered Configuration workbench with reciprocal setting consumer backlinks. Migration evidence now has a checked inventory for all 40 forward migrations, and restore tests verify staged dry-run/apply behavior, pre-restore database preservation, WAL/SHM cleanup, restored database content, and unsafe pending-path rejection. External-only validation now has a checked fake-evidence/operator-procedure inventory. Configuration operation parity, redaction, intelligence, verification-safety, documentation-policy, security/adversarial, unit/property/fuzz, and fixture/integration-failure boundaries now have checked evidence matrices. Full keyboard/browser E2E, broader migration/backup/restore/restart acceptance, streamed-provider event fuzzing, provider execution failure fakes, performance/soak evidence, and acceptance reporting remain open.

Current pause: implementation work is stopped after the I2-DOD-26 runnable Compose/acceptance coverage checkpoint. `I2-DOD-26` remains in progress, and the next safe implementation boundary is closing one of the remaining in-progress acceptance rows with executable evidence rather than adding another missing-row inventory.
...
The DOD-13 verification-safety boundary now has a checked matrix covering newly introduced finding non-waiver, immutable full-suite final verification policy, explicit golden-update approval, and REST/CLI/browser parity. This records existing safety proof without claiming full Increment 2 completion.

The DOD-14 documentation-policy boundary now has a checked in-progress matrix covering policy impact selection, source-controlled generation, render/check validation blockers, fresh verification and traceability, and operator parity. Known renderer/tool-profile and independent documentation-QC work remains explicit before completion.

The security/adversarial acceptance boundary now has a checked in-progress matrix covering authorization, stale edits, injection, worker isolation, SSRF/egress/fallback/cost, webhook authentication, cross-project isolation, restore path safety, no-write preview, and redaction evidence. Final acceptance remains open.

The I2-31.2 unit/property/fuzz boundary now has a checked in-progress matrix and seed fuzz entrypoints for implemented untrusted input surfaces. Streamed provider event fuzzing remains open until the provider gateway owns streamed chunk parsing.

The I2-31.3 fixture/integration failure boundary now has a checked in-progress matrix, expanded PHP npm/Vite/Tailwind fixture evidence, and direct provider denied-manifest regression coverage for cost/capability drift. Provider timeout/rate-limit/stream/batch execution fakes remain open until outbound provider execution is implemented.

The I2-31.4 local-fake E2E boundary now has a checked in-progress matrix mapping all 14 required workflow steps to existing deterministic coordinator, UI, API, CLI, provider, configuration, backup, and documentation evidence. The coordinator regression now verifies retained evidence graph records for the local publication workflow. One continuous browser-originated local-fake workflow through onboarding, pack acceptance, configuration rollback, provider route approval, fails-closed remote inference, backup/migrate/restore, and resume remains open.

The I2-31.5 performance/soak boundary now has a checked in-progress matrix covering scheduler resource safety and fairness, runtime benchmark quality gates, historical evaluation metrics, observability timing/resource diagnostics, and deterministic local acceptance smoke evidence. A bounded soak runner with retained threshold reports remains open.

The I2-DOD-26 runnable Compose/acceptance boundary now has a checked in-progress matrix covering CI, local acceptance scripting, documented Compose views, mock runtime stack startup/health, and maintenance scripts. A final retained full live acceptance run remains open until the remaining local acceptance rows are ready.
...
Revision note (2026-07-23 22:25Z): resumed Increment 2 Milestone 5 and added the versioned schema-constrained agent-contract foundation. Migration 26 records append-only validation evidence; implementation/QC model calls request strict JSON Schema response formats; workflow/API/CLI/browser surfaces expose schema versions, hashes, retry policy, compatible roles/modes, and validation history. Milestone 5 remains open for retry orchestration, Test Designer, golden/rehearsal gates, Documentation Agent/QC, and OPA-backed policy activation.

Revision note (2026-07-23 23:10Z): paused and synchronized Increment 2 Milestone 5 at the independent Test Designer checkpoint. Added migration 27, `test_designer` worker/model routing, exact contract/risk/result-bound task packets, strict `test_proposal` decoding, append-only report storage, workflow routing for medium/high risk, low-risk skipped evidence, REST/OpenAPI/generated-client/CLI/browser visibility, report context reuse, and updated coverage/docs. Validation passed for `go test ./...`, `go test -race ./...`, `go vet ./...`, frontend check:api/tests/typecheck/build/Redocly, and Compose validation.

Revision note (2026-07-23 23:35Z): resumed Increment 2 Milestone 5 and added the golden/rehearsal gate foundation. Added migration 28, `internal/golden`, append-only report and approval storage, the `golden_rehearsal_review` workflow state, registered capability/Test Designer rehearsal selection, fail-closed approval-required changed-golden behavior, API/OpenAPI/generated-client/CLI/browser evidence surfaces, QC context reuse, and updated quality workflow docs plus coverage. Validation passed for focused Go packages, `go test ./...`, frontend check:api/tests/typecheck, and Redocly lint.

Revision note (2026-07-23 23:39Z): advanced Increment 2 Milestone 5 with the Documentation Agent foundation. Added migration 29, `internal/docagent`, append-only documentation manifest storage, `documentation` worker/model routing, `loading_documentation_model` and `documentation_review` workflow states, exact contract/risk/result-bound manifest validation, source-controlled documentation commit handling with fresh verification, API/OpenAPI/generated-client/CLI/browser evidence surfaces, and updated quality workflow docs plus coverage. Validation passed for focused Go packages and frontend check:api/tests/typecheck/Redocly.

Revision note (2026-07-24 00:18Z): advanced Increment 2 Milestone 5 with the OPA policy lifecycle foundation. Added migration 30, `internal/policy`, append-only bundle/activation/simulation/decision storage, the `policy_review` workflow state, active bundle decision recording before QC, redacted policy inputs, API/OpenAPI/generated-client/CLI/browser evidence surfaces, and updated quality workflow docs plus coverage. Validation passed for focused Go packages, `go test ./...`, `go vet ./...`, frontend check:api/tests/build, OpenAPI lint, and Compose validation. Full Rego authoring, policy tests/coverage UI, rollback workbench, and cross-domain protected-action enforcement remain open.

Revision note (2026-07-24 00:48Z): paused at the operator-requested git sync point after adding embedded OPA/Rego advanced policy bundle authoring and retained test evidence. Added migration 31, OPA v1 module dependencies, formatted-source hashing, activation gating on retained passing tests, API/OpenAPI/generated-client/CLI create/test/test-runs operations, and a Policy page with simulation, coverage, activation history, source inspection, and expert Rego authoring. Validation passed for focused Go packages, `go test ./...`, `go test -race ./...`, `go vet ./...`, frontend check:api/tests/build, OpenAPI lint, and Compose validation. Increment 2 remains active and incomplete.

Revision note (2026-07-24 01:09Z): resumed Increment 2 Milestone 5 with Test Designer disposition enforcement. Added migration 32, `awaiting_test_design_disposition`, append-only proposal disposition storage, effective disposition overlays, API/OpenAPI/generated-client/CLI/browser actions, and workflow regression coverage that pauses before later quality gates until proposals are explicitly resolved. Focused Go and frontend checks pass; Increment 2 remains active and incomplete.

Revision note (2026-07-24 01:28Z): completed the bounded structured-output retry slice for Increment 2 agent contracts. The trusted model worker now retries implementation, QC, Test Designer, and Documentation Agent outputs with controller validation feedback and fails closed after three invalid attempts without applying edits or accepting malformed evidence. Focused tests passed for `internal/agents`, `internal/workflow`, and `cmd/maintainer-worker`; Increment 2 remains active and incomplete.

Revision note (2026-07-24 01:37Z): advanced the golden/rehearsal gate from terminal approval-required failure to an auditable operator wait. Added `awaiting_golden_approval`, latest append-only approval resolution, API transition back to documentation loading after all required comparisons are approved, OpenAPI/generated-client updates, and Jobs page approval/rejection controls. Focused Go packages, frontend tests/build, generated-client drift, and JSON validation pass; Increment 2 remains active and incomplete.
...
Revision note (2026-07-24 04:20Z): added the provider-gateway foundation. Migration 35 stores provider, endpoint, model, route, and append-only egress manifests; seeded defaults keep remote profiles disabled and local llama.cpp available; route simulations retain hashed allow/deny manifests after data-class, endpoint, structured-output, token, cost, and enabled-profile checks. REST/OpenAPI/generated-client, `maintainctl model-provider`, Models-page provider controls, docs, and coverage evidence are updated. Focused provider/storage/API/CLI tests and targeted frontend gates pass; Increment 2 remains active and incomplete.

Revision note (2026-07-24 04:25Z): paused at the operator-requested sync checkpoint after adding provider capability probes. Migration 36 stores immutable probe observations; deterministic fake adapters cover local llama.cpp, OpenAI Responses, OpenAI Chat Completions, OpenAI-compatible, Azure OpenAI, Anthropic Messages, Gemini, Vertex Gemini, Bedrock Converse, and Bedrock Responses-compatible families; API/OpenAPI/generated-client, CLI, Models-page probe controls, docs, and coverage evidence are current. Increment 2 remains active and incomplete.

Revision note (2026-07-24 04:42Z): advanced Milestone 6 with retained local runtime benchmark evidence. Migration 37 stores append-only benchmark runs and the model benchmark action records exact runtime identity, smoke quality/determinism gates, prompt/decode timing, duration, memory, cache eligibility, experimental feature gates, and non-activating recommendation evidence. `maintainctl model benchmarks`, OpenAPI/generated-client, the Models-page runtime lab, docs, and coverage map are updated. Increment 2 remains active and incomplete.

Revision note (2026-07-24 05:08Z): advanced Milestone 6 with the first typed evidence graph checkpoint. Migration 38 adds append-only graph nodes and edges; artifact indexing now retains job/artifact nodes and produced edges in the same transaction as the artifact and audit event; API, CLI, Jobs-page UI, docs, generated types, and coverage map are updated. The graph is not yet claimed complete because task-contract, verification, policy, documentation, QC, publication, SBOM, historical evaluation, and support-bundle edges still need to attach to the same model.

Revision note (2026-07-24 05:35Z): added the first historical evaluation checkpoint before the requested sync pause. Migration 39 retains immutable evaluation datasets and isolated run reports; the offline simulator hides known patches behind derived hashes, uses `eval://memory/...` and `eval://cache/...` namespaces, records deterministic profile metrics and report hashes, and blocks automatic model/policy/route promotion. REST/OpenAPI/generated-client, CLI, Evaluation-page UI, docs, and coverage evidence are current; real historical repair execution remains open.

Revision note (2026-07-24 06:20Z): paused at the requested sync checkpoint after adding the first observability/support-bundle foundation. Migration 40 retains append-only redacted telemetry events and bounded support bundle manifests; API/OpenAPI/generated-client, `maintainctl observability`, Observability-page UI, docs, and coverage evidence are current. External OTLP remains disabled/default-off and broader component instrumentation remains open.

Revision note (2026-07-24 06:45Z): started Milestone 7 with the 17-area dashboard information architecture checkpoint. The browser console now exposes the contract's named areas, adds dedicated Documentation and Security/SBOM pages, tests top-level navigation and accessibility with deterministic fixtures, and documents the page map in `docs/dashboard.md`. `go test ./internal/config` and the full frontend test/build/API-drift gate pass; final dashboard/config deep-linking and full E2E remain open.

Revision note (2026-07-24 06:55Z): advanced Milestone 7 by implementing dashboard-to-Configuration filtered links and Configuration-to-consumer backlinks. Hash route parsing now supports parameterized Configuration filters, setting cards expose feature/status consumers, and `docs/dashboard.md` records the navigation contract. Focused App and Configuration tests pass; full browser/keyboard E2E remains open.

Revision note (2026-07-24 07:20Z): advanced Milestone 7 migration and backup evidence. Added `config/migration-coverage.json`, `docs/migrations.md`, centralized schema-version/migration enumeration, and tests that cross-check embedded migrations, migration files, durable-record coverage, restart/resume evidence, backup/restore evidence, docs, and rollback guidance. Restore tests now prove preservation of the previous database and stale WAL/SHM cleanup. Increment 2 remains active and incomplete pending focused validation and broader acceptance.

Revision note (2026-07-24 07:35Z): added checked external-only validation coverage. `config/external-validation-coverage.json` maps the eight production-only acceptance areas to prerequisites, safe fake evidence, finite operator procedures, and CI non-claims; a config test verifies every referenced evidence/procedure path exists. Increment 2 remains active pending focused validation and final acceptance reconciliation.

Revision note (2026-07-24 07:55Z): added checked configuration operation parity coverage. `config/configuration-parity.json` maps the typed registry and legacy compatibility operations to OpenAPI routes, `maintainctl` commands, browser surfaces, docs, and regression tests; a config test verifies required operation coverage and referenced evidence. Increment 2 remains active pending focused validation and final acceptance.
...
Revision note (2026-07-24 09:45Z): added the checked security/adversarial inventory for `I2-28` and `I2-DOD-23`, plus an explicit unsafe pending-restore path regression. Authorization, stale-edit, injection, worker isolation, SSRF/egress/fallback/cost, webhook authentication, cross-project isolation, restore path safety, no-write preview, and redaction boundaries are now tied to machine-readable evidence; Increment 2 remains active.

Revision note (2026-07-24 10:05Z): added the checked unit/property/fuzz inventory for `I2-31.2` and real Go fuzz entrypoints for declarative imports, model manifests, forge webhooks, backup restore paths, untrusted Markdown task packets, and fake-model packet extraction. The row remains in progress because streamed provider event parser fuzzing is not implemented yet.

Revision note (2026-07-24 10:40Z): added the checked fixture/integration failure inventory for `I2-31.3`, extended the PHP fixture with npm/Vite/Tailwind files, and added a remote-provider denied-manifest regression for cost-circuit and capability-drift cases. The row remains in progress because provider timeout, rate-limit retry, streamed interruption, and batch partial-failure execution fakes are not implemented yet.
