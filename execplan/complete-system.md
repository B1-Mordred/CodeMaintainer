...
- Observation: the existing queue/schedule path retained dispatch history but not resource-decision reasoning.
  Evidence: migration 34 adds separate append-only `scheduler_decisions` rows for selected/deferred/rejected job IDs, co-residence safety, fairness, model batch group, and resource summary while leaving existing lease semantics unchanged.
...
Increment 2 Milestone 6 has started but remains open. The scheduler now has safe defaults, deterministic simulation, retained decision evidence, and API/CLI/browser parity. Live queue leases still use the existing dispatcher and must be transactionally bound to scheduler decisions before the scheduler DOD can close.
