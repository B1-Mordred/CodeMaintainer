...
- [ ] Milestone 6 product completion (completed through 2026-07-20 18:40Z: one-time administrator bootstrap; Argon2id password storage; random hash-only server sessions and CSRF tokens; HttpOnly SameSite cookies with explicit TLS-mode secure flag; 12-hour expiry; bounded authentication attempts; exact non-hierarchical operator/reviewer RBAC; server-recorded five-minute reauthentication; append-only bootstrap/login/logout/reauth audit; forged-header rejection; authenticated CLI session file; accessible bootstrap/login UI; a responsive safe/expert operations console with all ten required page areas and lazy typed API reads; API-backed project registration, jobs, memory, scheduling, user creation, backup creation, and restore validation controls; Git-bridge project registration/sync and credential-safe diagnostics; model-supervisor list/status/benchmark APIs; the full named maintainctl command surface including file-backed tasks; host CPU/architecture/disk detection and benchmark guidance; local Prometheus-format job/uptime metrics; recently reauthenticated user administration with exact roles, session revocation, and last-administrator protection; and consistent checksum-manifested backup bundles that include SQLite/audit/config state plus selected artifacts and OpenViking state, exclude disposable/reproducible content, enforce size bounds, and require a private 256-bit production encryption key. Remaining: complete finding controls and richer live page data, offline restore application, updates and rollback, full diagnostics/observability detail, full accessibility/browser pass, documentation, SBOM/license inventory, hardened optional profiles, and the final acceptance suite).
...
Milestones 1 and 3 are closed, and the local-provider portion of Milestone 4 is operational. The repository produces a non-root distroless controller serving a generated-client React dashboard; persists jobs, transitions, phases, artifacts, findings, approvals, configuration, and audit data across restarts and migrations; and reconciles runner, model, Git, and workflow side effects through narrow authenticated interfaces. A live mock stack synchronized a seeded bare repository, reproduced and repaired its defect, made two deterministic commits, persisted one blocking QC lifecycle through closure, paused at the exact repaired SHA, survived a controller rebuild, accepted an exact-SHA reviewer approval, and published that branch once to the local bare remote. Separate container acceptance proves the implementation and QC image boundary, while the default end-to-end lifecycle remains deterministic and credential-free. Production browser authentication, real GitHub App behavior, durable memory/Hermes, the complete dashboard, backup/update operations, and their final audit are not yet claimed.
