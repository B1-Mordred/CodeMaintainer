...
Increment 2 Milestone 7 has started but remains open. The operator console now uses the required 17 named areas, keeps setup/health/administration operations available, and adds dedicated Documentation and Security/SBOM pages backed by controller-retained policy, manifest, capability-pack, finding, rehearsal, and release-evidence data. Final configuration deep-linking, keyboard/browser E2E, migration/backup/restore/restart acceptance, security/property/fuzz/isolation coverage, performance/soak evidence, and acceptance reporting remain open.
