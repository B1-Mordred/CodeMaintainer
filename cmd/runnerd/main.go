...
func run(logger *slog.Logger) error {
	if env("RUNNERD_PROFILE", "mock") != "mock" {
		return errors.New("only the explicit mock executor profile is implemented; refusing an implicit production fallback")
	}
	...
	service, err := runnerd.NewService(policy, runnerd.NewFakeExecutor(), token, logger)
	if err != nil {
		return err
	}
	...
	go func() {
		logger.Info("runnerd listening", "network", "unix", "profile", "mock")
