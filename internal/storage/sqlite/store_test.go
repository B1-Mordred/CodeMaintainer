...
func TestMigrationFromVersionOneAddsEveryRetainedSchema(t *testing.T) {
	...
	if migrations != CurrentSchemaVersion {
		t.Fatalf("applied migration count = %d, want %d", migrations, CurrentSchemaVersion)
	}
	...
	for _, table := range []string{"provider_profiles", "endpoint_profiles", "model_profiles", "route_profiles", "provider_egress_manifests"} {
		var name string
		if err := store.db.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name); err != nil {
			t.Fatalf("migration 35 table %s missing: %v", table, err)
		}
	}
	for _, table := range []string{"provider_capability_probes"} {
		var name string
		if err := store.db.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name); err != nil {
			t.Fatalf("migration 36 table %s missing: %v", table, err)
		}
	}
	for _, table := range []string{"runtime_benchmarks"} {
		var name string
		if err := store.db.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name); err != nil {
	...
	for _, table := range []string{"evaluation_datasets", "evaluation_runs"} {
		var name string
		if err := store.db.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name); err != nil {
			t.Fatalf("migration 39 table %s missing: %v", table, err)
		}
	}
	for _, table := range []string{"observability_events", "support_bundles"} {
		var name string
		if err := store.db.QueryRowContext(ctx, "SELECT name FROM sqlite_master WHERE type='table' AND name=?", table).Scan(&name); err != nil {
			t.Fatalf("migration 40 table %s missing: %v", table, err)
		}
	}
}
