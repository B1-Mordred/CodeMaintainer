...
func TestAllRequiredStatesAreUniqueAndValid(t *testing.T) {
	want := []State{
		...
		StateAwaitingTestDesignDisposition, StateGoldenRehearsalReview,
	...
	paths := [][]State{
		{
			...
			StateGoldenRehearsalReview, StateAwaitingGoldenApproval,
			StateLoadingDocumentationModel, StateDocumentationReview,
			StateAwaitingRemoteEgressApproval, StateDocumentationReview,
			StatePolicyReview,
		...
		},
		{
			...
			StateFinalVerification, StateLoadingDocumentationModel, StateDocumentationReview, StatePolicyReview, StateLoadingQCModel, StateQCReview,
...
func TestDangerousShortcutsAreRejected(t *testing.T) {
	for _, tc := range []struct{ from, to State }{
		...
		{StateAwaitingRemoteEgressApproval, StatePublishingBranch},
